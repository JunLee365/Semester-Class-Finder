...
function writeSearch(req, res){
    ...
    if(filter == "allFields"){
        sql = `SELECT * FROM courses
            WHERE Subject   LIKE '%` + search + `%' OR Course          LIKE '%` + search + `%' OR
            CourseName      LIKE '%` + search + `%' OR Component       LIKE '%` + search + `%' OR
            Section         LIKE '%` + search + `%' OR Days            LIKE '%` + search + `%' OR
            StartTime       LIKE '%` + search + `%' OR EndTime         LIKE '%` + search + `%' OR
            StartDate       LIKE '%` + search + `%' OR EndDate         LIKE '%` + search + `%' OR
            Duration        LIKE '%` + search + `%' OR InstructionMode LIKE '%` + search + `%' OR
            Building        LIKE '%` + search + `%' OR Room            LIKE '%` + search + `%' OR
            Instructor      LIKE '%` + search + `%' OR
            EnrollCap       LIKE '%` + search + `%' OR WaitCap         LIKE '%` + search + `%' OR
            CombDesc        LIKE '%` + search + `%' OR CombEnrollCap   LIKE '%` + search + `%';`;}
    // search course num
    ...
    con.query(sql, function(err, result){
        ...
        for (let item of result) {
            ...
            <button name = "add" value="` + item.id + `"> Add CSE ` + item.Course + ` - ` +
            item.CourseName + ` - ` + item.Component + ` - Section ` + item.Section + `</button></form>
            <pre> Days: ` + item.Days + `
...
function constructSQLDayCommand(search) {
    sql = `SELECT DISTINCT * FROM saved WHERE Days like '%` + search + `%' ORDER BY STR_TO_DATE(StartTime, '%l:%i %p');`;
    return sql;
};
